import Notification from "./icons/bell.svg?url";
// import Alarm from "./icons/alarm-clock.svg?url";
import Moon from "./icons/moon.svg?url";
import Sun from "./icons/sun.svg?url";
import Analytics from "./icons/analytics.svg?url";
import LayoutDashboard from "./icons/layout-dashboard.svg?url";
import Book from './icons/book-open.svg?url';
import Sparkle from './icons/sparkle.svg?url';
import Bookmark from './icons/bookmark-minus.svg?url';
import Settings from './icons/settings.svg?url';
import Search from './icons/search.svg?url';
import Alert from './icons/alert.svg?url'
import ArrowDown from './icons/chevron-down.svg?url'
import Check from './icons/check.svg?url'
import Close from './icons/x.svg?url'
import Sparkles from './icons/sparkles.svg?url'
import Messsage from './icons/message.svg?url'
import Warehouse from './icons/warehouse.svg?url'
import Positive from './icons/Positive.svg?url'
import File from './icons/file.svg?url'
import ProductIcon from './icons/productIcon.svg?url'
import Flight from './icons/plane.svg?url'
import Exchange from './icons/arrow-right-left.svg?url'
import NewsPaper from './icons/newspaper.svg?url'



export {
    Notification,
    // Alarm,
    Alert,
    Moon,
    Sun,
    Analytics,
    LayoutDashboard,
    Book,
    Sparkle,
    Bookmark,
    Settings,
    Search,
    ArrowDown,
    Check,
    Close,
    Sparkles,
    Messsage,
    Warehouse,
    Positive,
    File,
    ProductIcon,
    Flight,
    Exchange,
    NewsPaper
}